...
st.write("Leverage advanced multi-modal AI to parse Single-Line Diagrams (SLDs), run strict compliance audits, or consult the electrical code assistant.")
...
tab1, tab2 = st.tabs(["🚀 Automated Substation Compliance Audit", "💬 Electrical Code Copilot"])

# =========================================================================
# TAB 1: AUTOMATED SUBSTATION COMPLIANCE ENGINE
# =========================================================================
with tab1:
    st.header("SLD Safety & Validation Automation")
    st.write("Upload an Electrical Single-Line Diagram (SLD) to perform a strict Pass/Fail validation audit against standard protection rules.")

    uploaded_file = st.file_uploader("Upload Single-Line Diagram (PNG, JPG, JPEG)", type=["png", "jpg", "jpeg"], key="sld_uploader")

    # Strict Compliance Engineering prompts inside the automation logic
    PROMPT_1 = """
    ...
    CRITICAL INSTRUCTION: Do not exaggerate or create hypothetical risks. If a zone has appropriate visible or logically implied engineered safeguards (such as standard overcurrent relays, fuses, or surge arresters for its voltage level), you must explicitly mark it as compliant. Only flag a row as a 'RISK DETECTED' if there is a clear protection gap, missing critical hardware safeguard, or a severe unmitigated operational hazard.

    Evaluate each zone for these exact conditions:
    1. Short Circuit / Overcurrent Protection (e.g., missing breakers or protective relays where paths tie)
    2. Overvoltage / Lightning Surge Protection (e.g., transformer primary exposed without surge arresters)
    3. Loss of Protection Coordination (e.g., a critical bus lacking any downstream isolation or isolation elements)
    ...
    [
      {
        ...
        "Required Engineered Safeguard": "None (if Compliant) OR the specific physical hardware component required to close the gap",
        "Targeted Action Item": "Clear, concise engineering recommendation or validation note"
      }
    ]
    """
    ...
    if uploaded_file:
        ...
        if st.button("🚀 Run Compliance Validation Audit", type="primary"):
            try:
                image_bytes = uploaded_file.read()
                pil_image = Image.open(io.BytesIO(image_bytes))
                
                with st.status("Processing Electrical Validation Pipeline...", expanded=True) as status:
                    ...
                    status.update(label="🛡️ Verification in progress: Auditing circuit elements against codes...", state="running")
                    ...
                    status.update(label="🎉 Compliance check complete! Rendering data worksheet...", state="complete")

                st.session_state["final_df"] = pd.DataFrame(final_hazop_rows)
                st.balloons()

            except json.JSONDecodeError:
                st.error("🛑 The AI model output did not form a clean, verifiable data structure. Please try running the validation audit again.")
            except Exception as e:
        ...
        if "final_df" in st.session_state:
            ...
            st.success("📝 Validation Matrix Compiled Successfully!")
            
            st.subheader("Preview Substation Compliance Report")
            st.dataframe(df)
            ...
            st.download_button(
                label="📥 Download Excel Validation Audit Sheet",
                data=excel_buffer,
                file_name=f"Electrical_Validation_Report_{st.session_state['current_file']}.xlsx",
